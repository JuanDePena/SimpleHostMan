...
work_dir="$(mktemp -d)"
staging_dir="${work_dir}/simplehost-control-${version}"
bundle_dir="${repo_root}/dist/releases"
bundle_path="${bundle_dir}/simplehost-control-${version}.tar.gz"
...
tar -C "${work_dir}" -czf "${bundle_path}" "simplehost-control-${version}"
