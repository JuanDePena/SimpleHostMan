...
function printHelp() {
  console.log(
    [
      "Usage:",
      "  node scripts/set-version.mjs <YYMM.DD.HH>",
      "  node scripts/set-version.mjs --now",
      "  node scripts/set-version.mjs --print-now",
      "",
      "Examples:",
      "  node scripts/set-version.mjs 2604.19.02",
      "  node scripts/set-version.mjs --now"
    ].join("\n")
  );
}
...
function resolveRequestedVersion(argv) {
  ...
  if (requested === "--print-now") {
    console.log(computeNowVersion());
    process.exit(0);
  }

  const version = requested === "--now" ? computeNowVersion() : requested;

  if (!versionPattern.test(version)) {
    throw new Error(`Invalid version "${version}". Expected YYMM.DD.HH.`);
  }
