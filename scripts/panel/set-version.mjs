...
function resolveRequestedVersion(argv) {
  const requested = argv[2] === "--" ? argv[3] : argv[2];

  if (!requested) {
    throw new Error("Usage: node scripts/set-version.mjs <YYMM.DDNN|--today>");
  }

  const version = requested === "--today" ? computeTodayVersion() : requested;

  if (!versionPattern.test(version)) {
    throw new Error(`Invalid version "${version}". Expected YYMM.DDNN.`);
  }
